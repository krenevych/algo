...
def wave(maze, start, wall_cell):
    ...
    n = len(maze)     # кількість рядків у матриці P5_Maizes
    m = len(maze[0])  # кількість стовпчиків у матриці P5_Maizes
...
def drawWayInMatrix(source, start, end, TAG):
    ...
    n = len(source)  # кількість рядків у матриці P5_Maizes
    m = len(source[0])  # кількість стовпчиків у матриці P5_Maizes
    # створення та ініціалізація хвильової матриці
    # такої ж розмірності, що і матриця лабіринту
